...
### Changing the Bias
Increasing the bias associated with the neuron is the simplest way to increase its activation.

Unlike changing the weights or the activations from the previous layer, the effect of a change to the bias on the weighted sum is constant and predictable. As far as this specific training example of a 2 is concerned, the bias should be increased to increase the activation.
