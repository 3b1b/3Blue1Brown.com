...
And presumably, if the idea of an average height among all infinitely many points is going to make any sense at all, the more points we sample, which would involve adding up more heights, the closer the average of that sample should be to the actual average of the continuous variable. This should feel at least somewhat related to taking an integral of $\sin(x)$ between $0$ and $\pi$, even if it might not be clear at first exactly how the two ideas will match up.
...
So if we write the spacing between samples as $dx$, the number of samples is $\frac{\pi}{dx}$. So replacing the denominator with $\frac{\pi}{dx}$ here, you can rearrange, putting the $dx$ up top and distributing. 
...
TODO: comprehensive question for the equation shown in the introduction.

<Question
  question="What is the average height of the function $f(x) = 0.1x^{3} - 0.9x^{2} + 1.8x + 4$ from $1$ to $6$?"
  choice1="1"
  choice2="2"
  choice3="3"
  choice4="4"
