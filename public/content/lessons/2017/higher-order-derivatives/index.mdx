...
TODO: comprehensive /review question: what is the slope at $x=4$ and $x=2$?
...
A graph like this has a very positive second derivative at the input $4$, since the slope is rapidly increasing around that point. Whereas another graph like the one belows still has a positive second derivative at that same point, but it's smaller, since the slope is increasing only slowly.
...
TODO: footnote about min / max problems and finding the zeros of derivatives leading to optimization problems or connection with the physical intuition.
...
TODO: pi creature astronaut in an space example.
