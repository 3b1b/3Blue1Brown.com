...
<Question
  question="For example, if $x=2$ and $dx$ was $0.01$, approximately how much bigger would the new area be relative to the size of $dx$?"
  choice1="6"
  choice2="2"
  choice3="4"
  choice4="3"
  answer={3}
  explanation="If $x=2$ and $dx$ is $0.01$, the area of the thin rectangles would be $2 \cdot 2 \cdot 0.01$ plus the small area of $dx^2$ which is about $4$ times the size of $dx$."
/>

<!-- TODO: check answer against graph once Question children is implemented -->
...
Alternatively, the question can be modeled as volume, where instead of scaling one of the sides, the $3$ extends the geometry into another dimension.
