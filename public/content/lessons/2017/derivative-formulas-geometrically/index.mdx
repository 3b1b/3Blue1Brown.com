...
<Figure
  image="figures/2.08-detail.svg" 
  caption="The tangent line at $x=2$ for $f(x)=x^2$"
/>
...
<Question
  ...
  answer={1}
  explanation="The rate of change in the area per unit of length added, $\frac{d(x^2)}{dx}$, would be $2 \cdot 3$, or $6$. As another example, if you were starting at $x=5$, that rate of change would be $10$ units of area per change in $x$."
/>
...
<Figure
  ...
  caption="The length of $dx$ is illustrated as rather large in this figure, remember that we are interested in what happens as $dx$ approaches $0$."
/>
...
Let's look at the pattern the first three monomial functions form, where I've included the monomial of degree one for completeness. So far, each of these monomial functions has had a nice geometric meaning. Nudging the input $x$ by a small amount $dx$ has allowed us to see how the geometry changes and find the deriative. <!-- TODO: link to footnote that is a portal into radius of convergence --> 

<Figure
  image="figures/8.11-9.35-base-cases.svg"
  caption="Expressions that contain $dx^2$ we are able to ignore, because we are interested in the rate of change of the functions geometry as $dx$ approach 0."
/>

However, from a geometric perspective, we have hit a roadblock. How do we visualize four dimensions? One path forward is to continue with algebra, using our geometric intuition to inform the base cases. For example, we can expand higher degree monomial expressions, using the rules of algebra, where the input $x$ have been nudged by a small amount $dx$. This gives us the following series of expressions.
...
Focusing on the rate in change introduced by the very small nudge and ignoring expressions containing $dx^2$, the pattern that emerges is what's known in the business as the "power rule". Given a monomial function raised to some power, $n$, applying the rule gives us the derivative of the function.
...
<Accordion title="Geometry interpretation for derivative of $f(x)= 3x^2$">

This question can be modeled as a question of change in area. For example, nudging the input of the function by $dx$ gives the expression $3(x+d x)^{2}$

<Figure
  image="figures/power-rule-example.svg" 
  caption="Power Rule Example"
/>

</Accordion>

<Accordion title="Algebraic interpretation of $f(x)= 3x^2$">

This question can be modeled as a question of change in area
...
In the first chapter, we found a formula for the area of a circle using a small nudge to the radius of the circle. 

We know the circumference of a sphere with radius $$r$$ is $$2\pi r$$. 
...
<Accordion title="Volume of Sphere Visual Explanation">
...
<Figure
  ...
  caption="The volume of the sphere can be visualized as a collection of shells whose volume is given by their surface area multiplied by their thickness $dr$."
/>
...
So think back to the walk around the unit circle, having traversed an arc length of theta, where $\sin(\theta)$ is the height of this point. Consider a slight nudge of $d$-theta along the circumference of the circle; a tiny step in your walk around the unit circle. How much does this change $\sin(\theta)$? How much does that step change your *height* above the x-axis? This is best observered by zooming in on the point where you are onthe circle.
...
Now, think about what the derivative of sine is supposed to mean. It’s the ratio between that $d\left(\sin(\theta)\right)$, the tiny change to the value of sine, divided by $d \theta$, the tiny change to the input of the function. From the picture, that’s the ratio between the length of the side adjacent to this little angle divided by the hypotenuse. Well, let’s see, adjacent divided by hypotenuse; that’s exactly what $\cos(\theta)$ means!
