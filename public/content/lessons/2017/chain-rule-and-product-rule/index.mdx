...
For example, let's think about this function $f(x) = \sin(x) + x^2$. It's a function where for function where, for every input, you add together the values of $\sin(x)$ and $x^2$ at that point.
...
That is, the function $x^2$ gets you from line $1$ to line $2$, and the function sine gets you from line $2$ to line $3$.
...
As I shift that value of $x$, maybe up to the value $3$, then the value on the second shifts to whatever $x^2$ is, in this case $9$. And that bottom value, being the $\sin(x^2)$, will go over to whatever the $\sin(9)$ is.

<Figure
  image="figures/chain-rule-x-3.svg" 
/>

So for the derivative, let’s again think of nudging that $x$-value by some little $dx$, and I always think it’s helpful to think of $x$ starting as some actual number, maybe $1.5$. And $dx$ as some small number that I imagine approaching zero, maybe $0.1$.

<Figure
  image="figures/chain-rule-dx.svg" 
/>

The resulting nudge to this second value, the change to $x^2$ caused by such a $dx$, is what we might call $d(x^2)$. You can expand this as $2x \cdot dx$, which for our specific input that length would be $2(1.5)dx$, but it helps to keep it written as $d(x^2)$ for now. In fact let me go one step further and give a new name to $x^2$, maybe $h$, so this nudge $d(x^2)$ is just $dh$.
...
Now think of that third value, which is pegged at $\sin(h)$. It’s change $d\left(\sin(h)\right)$; the tiny change *caused* by the nudge $dh$. Well, we know the derivative of sine, so we can expand $d\left(\sin(h)\right)$ as $\cos(h) \cdot dh$; that’s what it means for the derivative of sine to be cosine.
...
Notice, for the derivative of $g$, I’m writing it as $\frac{dg}{dh}$ instead of $\frac{dg}{dx}$. On the symbolic level, this serves as a reminder that you still plug in the inner function to this derivative. But it’s also an important reflection of what this derivative of the outer function actually represents.
...
So in this chain rule expression, we’re saying look at the ratio between a tiny change in $g$, the final output, and a tiny change in $h$ that caused it, $h$ being the value that we’re plugging into $g$. Then multiply that by the tiny change in $h$ divided by the tiny change in $x$ that caused it.
...
Setup three numberlines to represent the function composition and nudge the input $x$ by a small amount $dx$. Remember we are interested in finding $df$, the change to the function represented on the bottom number line.

<Figure
  image="figures/exercise-1-chain-rule-setup.svg"
/>

Substitute $h=\sin(x)$ into the function composition. This simplifies $(\sin (x))^{2} = (h)^{2}$ and we know the change of the function $h^2$ given some small nudge $dh$ is $2h \cdot dh$.
...
From past chapters we know that the expression $d(\sin(x))$ is $\cos(x) \cdot dx$. Using this information we can express the chang ein area of the function as shown below. As before, the small rectangle's area in the bottom right corner is negligible.
