...
Now, given that we're actually showing you the full transformation, you could have just looked to see that v has coordinates [5, 2], but the cool part here is that this gives us a technique to deduce where the vector lands without needing to watch the transformation.
...
One way to imagine it is to first move i hat to [1, 2], then move j hat to [3, 1], always moving the rest of space in such a way that keeps the grid lines parallel and evenly spaced.

TODO: visual i moves

TODO: visual j moves

TODO: visual together
...
Let $A$ be the transformation matrix we are solving for, in the form:

$$
A=\left[\begin{array}{ll}a & b \\ c & d\end{array}\right]
$$

Next, we set up the systems of equations based on the given transformations. The transformations can be expressed as matrix equations:
...
Expanding these matrix equations by applying the matrix to the input vector we get the following equations.
...
Solve this system of equaitons to findthe values of $a, b, c$, and $d$ :

- From $-2 a+b=0$, we can express $b$ in terms of $a$ as $b=2 a$.
- Substituting $b=2 a$ into $2 a+3 b=8$, we solve for $a$, which gives us $a=1$.
- Plugging $a=1$ back into $b=2 a$, we find $b=2$.
- Using similar substitutions and solving methods for $c$ and $d$ in the equations $-2 c+$ $d=3$ and $2 c+3 d=1$, we find $c=-1$ and $d=1$.

In conclusion, the transformation matrix which is described by the behavior $\left[\begin{array}{c} 2 \\ 3 \end{array}\right] \to \left[\begin{array}{c} 8 \\ 1 \end{array}\right]$ and $\left[\begin{array}{c} -2 \\ 1 \end{array}\right] \to \left[\begin{array}{c} 0 \\ 3 \end{array}\right]$ is defined as $\left[\begin{array}{cc}1 & 2 \\ -1 & 1\end{array}\right]$.
