...
From the last chapter, we reasoned through what matrix multiplication should look like by following where the basis vectors $\hat{\imath}$, $\hat{\jmath}$, and $\hat{k}$ land after applying one transformation and then the other, because where the original basis vectors land will make up the columns of the composed matrix.
...
0 \\
-1
\end{array}\right]+0\left[\begin{array}{c} 1 \\ 1 \\ 0 \end{array}\right]+0\left[\begin{array}{c} 1 \\ 0 \\ 1 \end{array}\right]\right) \\ \rule{0pt}{2.5em}
& =\left[\begin{array}{ccc}
0 & 2 \\
0 & 2 & 0 \\
-2 & 0 & 0
...
We can double check our work visually to see that the basis vectors do indeed correspond to the columns of the resulting matrix.
