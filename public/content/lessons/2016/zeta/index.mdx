...
This offers a very beutiful way to think about plugging in $2+i$ into the zeta function. Start by thinking of each term $\frac{1}{1^2}$, $\frac{1}{2^2}$, $\frac{1}{3^2}$, etc., perhaps visuzlized as little lines whose lengths are the reciprocals of square numbers.
...
Let's try visualizing this function. It takes in inputs on this right half of the complex plane, and spits out outputs somewhere else on the complex plane. Anice way to understand complex functions is to picture them as transformations, meaning you look at each input to the function, and let it move over to its corresponding output.
