...
In the next chapter, we get into one of the most important topics in linear algebra: <LessonLink id="linear-transformations">Linear Transformations</LessonLink>. See you there!
...
The vector $\vec{\mathbf{u}} = \left[\begin{array}{c}  0  \\  -2  \\  4  \end{array}\right]$ is the only vector that can be expressed using just $\vec{\mathbf{v}}$ and $\vec{\mathbf{w}}$.
