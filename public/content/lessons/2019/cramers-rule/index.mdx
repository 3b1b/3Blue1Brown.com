...
So this is a sort of puzzle, what input $\left[\begin{array}{c} x \\ y \end{array}\right]$, is going to give you this output $\left[\begin{array}{c} -4 \\ -2 \end{array}\right]$?
...
So, the area of this parallelogram is sort of a screwy roundabout way to describe the vector's $y$-coordinate; it's a wacky way to talk about coordinates, but run with us.
...
For example, if you look the parallelogram spanned by the vector were your first basis vector lands, which is the first column of the matrix, and the transformed version of $\left[\begin{array}{c} x \\ y \end{array}\right]$, what is its area?
