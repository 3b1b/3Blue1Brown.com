...
The key feature of $e^x$ is that it's a function whose derivative equals itself, which makes it a useful building block for describing functions whose derivatives don't necessarily equal themselves, but which depend on themselves in more complicated ways.
...
If it’s initial length needed to be 0.3, the coefficient would be 0.3 times that amount. You get the idea.
...
<Question 
  question="Assuming we can write $f(t)$ as $\sum_{n = -\infty}^\infty c_n e^{n \cdot 2\pi i t}$, which of the following describes $f(t) e^{-2 \cdot 2 \pi i t} dt$?"
...
[^6]: Typically in math, you have to be more careful making a swap like this when infinities are involved. Again, such questions are what real analysis is built for. In this case, you can indeed swap the integral and the sum.
