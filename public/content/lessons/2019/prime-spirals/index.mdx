...
<Figure image="reside_class_mod_6.png" width="80%" />
...
<Question
  ...
  choice6="$6k+5$"
  answer={4}
  explanation="The number 381 has a quotient of 63 and a remainder of 3, so it belongs to the $6k+3$ residue class."
/>
...
<Question
  ...
  answer={3}
  explanation="$\sin(60\ rad) < 0$ and $\cos(60\ rad) < 0$, placing the start of the curve in the 3rd quadrant. If your calculator is in degrees mode, you will get the 1st quadrant by mistake!"
/>

In reality, with a little further zooming, you can see that there is actually a gentle spiral to these, but the fact that it takes so long to become prominent is a wonderful illustration, maybe the best illustration I've seen, for just how good an approximation $\frac{710}{113}$ is for $2\pi$. [[1](#Footnotes)]
...
<PiCreature emotion="angry" flip={true} text="We're running out of symbols!" />
...
<Question
  ...
  choice5="$\frac16$"
  answer={1}
  explanation="$ \lim_{x\to\infty}\left(\frac{\color{blue}\pi\color{black}(x;6,1)} {\color{blue}\pi\color{black}(x)}\right) = \frac1{\phi(6)} $ and we know from counting the visible arms on the smallest spiral that $\phi(6)=2$"
/>
...
As a quick sidenote, for those of you in the know, this is because of a large number that shows up in the continued fraction of $pi$. If you don’t know what that means, don’t worry about it! [↩](#Ingredients-for-a-Spiral-PI)

[2] Goldston, Pintz, Yildirim. [Primes in Tuples 1](https://arxiv.org/pdf/math/0508185.pdf).

​      Zhang. [Bounded gaps between primes](https://annals.math.princeton.edu/wp-content/uploads/Yitang_Zhang.pdf). [↩](#Dirichlet-s-Theorem)