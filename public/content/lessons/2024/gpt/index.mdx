...
Keeping score on our total number of parameters, the unembedding matrix has one row for each word in the vocabulary, giving 50,257 words, and each row has the same number of elements as the dimension of the embedding, giving 12,288 columns. It’s very similar to the embedding matrix, just with order swapped, so it adds another 617M parameters to the network, making our parameter count so far a little over a billion; a small but not insignificant fraction of the 175 billion that we'll end up with in total.

<Accordion title={
    <>
        Comprehension Question 4: If the model has a predefined vocabulary of 32,000 tokens and the dimension of the embedding matrix is 10,000, the dimension of the umembedding matrix is: <br />
        A. 10,000 x 32,000 <br /> 
        B. 320,000,000 <br /> 
        C. 32,000 x 10,000 <br /> 
        D. 175,000,000,000
    </>
} 
children={
    "C. 32,000 x 10,000. The unembedding matrix has one row for each token in the predefined vocabulary, giving 32,000 rows. Each row in the unembeedding matrix also has the same number of elements as the dimension fo the embedding matrix, giving 10,000 columns"
    }
/>
...
The idea is that if we want a sequence of numbers to serve as a probability distribution, say a distribution over all possible next words, all the values should be between 0 and 1 and should all add up to be 1. However, in deep learning, where everything we do looks like a matrix-vector product, the outputs we get by default won’t abide by this at all. The values are often negative, or bigger than 1, and they almost certainly don’t all add up to 1.
...
The way it works is to first raise e to the power of each number, which gives a bunch of positive values. Then you take the sum of all these new terms and divide each term by that sum, which normalizes it into an list which adds up to 1.
...
In some cases, such as when ChatGPT is using this distribution to sample each next word, we can add a little extra spice into this softmax function with a constant T inserted into all these exponents. It's called T since it vaguely resembles the role of temperature in certain thermodynamics equations. The effect of this constant is that when T is larger, more weight is given to lower values, making the distribution more uniform. When T is smaller, the larger values will dominate the distribution, and in the extreme setting when T is equal to 0, all the weight goes to the maximum value.

<Figure image="temperatureReal.png" width="100%"/>

Here's another bit of jargon:  
In the same way that we might call the components of the output of this function _probabilities_, people often refer to the components of the input the inputs as _logits_. So when you feed in some text, have the word embeddings flow through the network, and do this final multiplication by the unembedding matrix, machine learning people would refer to the components of that raw unnormalized output as the _logits_ for the next word prediction.

### And Thats The Basics

Our goal for this chapter was to lay the foundations for understanding the attention mechanism. If we have a strong intuition for _word embeddings_, _softmax_, how _dot products_ measure similarity, and the underlying premise that most calculations look like matrix multiplication with matrices full of tunable parameters, then understanding the attention mechanism, one of the keystone pieces of the whole modern boom in AI that we will explore in the next chapter, should be relatively smooth.
