...
Compared to attention, the computations in the MLPs are relatively simple, as they're essentially a pair of matrix multiplications with a simple function in between. However, interpreting what these computations are actually doing is exceedingly challenging. 
...
As we go through the MLP block, I will assume that you've either watched/read the last two chapters or have a basic sense of what a transformer is. But if not, here is a recap:
...
In the last chapter, we've already covered how attention blocks let us incorporate context, but a majority of the model parameters actually live inside the MLP blocks. A common thought for what these MLP blocks might be doing is that they offer _an extra capacity to store facts_. 

### Assumptions

Now before we dive into this example of how exactly the MLP could store the fact that _Michael Jordan plays basketball_, we're going to have to make a couple of assumptions about the high-dimensional space that we're working in.
...
Since the text Michael Jordan spans two different tokens, _Michael_ and _Jordan_, this would also mean we have to assume that an earlier attention block has successfully passed information to the second of these two vectors, _Jordan_, to ensure that it can encode both names.
...
Notice how the value of this dot product would be 2 if the vector encodes the full name _Michael Jordan_, and otherwise it would be less than or equal to 1.
...
And if you have a bias in this case, it's something that's added on regardless of the neuron values. For now, feel free to ignore it, as it's kind of hard to understand exactly what it's doing, just like all other parameter-filled objects. We'll label this down projection matrix as $$W_\downarrow$$ and this bias vector as $$B_\downarrow$$.
...
But where it gets interesting is when you relax that constraint a little bit and you tolerate some noise. Say you allow those features to be represented by vectors that aren't exactly perpendicular; they're just nearly perpendicular, maybe between _89_ and _91_ degrees apart.
