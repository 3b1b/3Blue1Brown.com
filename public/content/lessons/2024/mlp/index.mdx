...
In December of 2023, a few researchers from Google DeepMind posted about work on this question, using this same specific example of matching athletes to their sports. Although a full mechanistic understanding of _how facts are stored_ remained unsolved, the researchers had reported some interesting partial results, including the very general high-level conclusion that facts seem to be stored inside a specific part of these networks known as the **multi-layer perceptrons**(_MLPs_ for short). 

<Figure image="MLP.png" width="80%"/>

In the last few chapters, you and I have been digging into the details behind _transformers_-the architecture underlying large language models as well as a lot of other modern AI. 

In the previous chapter, we were focusing on a piece called **attention**, the block on the left of the image. The next step for us is to explore the details of what happens inside these **multi-layer perceptrons**, which make up the other big portion of the network.
...
As we go through the MLP block, I will assume that you've either watched/read the last two chapters _or_ have a basic sense of what a transformer is. 

But as a refresher, here's a quick reminder of the overall flow:
...
Then each token is associated with a high-dimensional vector, which is to say a long list of numbers, called it's **embedding**.
...
<Figure image="manylayers.png" width="100%" caption="There is also a _certain_ normalization step in between, but we'll leave that for another lesson."/>
...
In the last chapter, we've already covered how attention blocks let us incorporate context, but a majority of the model parameters actually live inside the MLP blocks. 

A common thought for what these MLP blocks _might_ be doing is that they offer _an extra capacity to store facts_. 
...
**First**, we'll suppose that one of the directions in this high-dimension space represents the idea of a first name _Michael_; another _nearly perpendicular_ direction represents the idea of the last name _Jordan_; and a third, also nearly perpendicular, direction that represents the idea of _basketball_:
...
For simplicity, we'll completely ignore the _very reasonable question_ of what it might mean if that dot product was bigger than one.
...
Since the text Michael Jordan spans two different tokens, _Michael_ and _Jordan_, this would also mean we have to assume that an earlier **attention block** has successfully passed information to the second of these two vectors, Jordan, to ensure that it can encode _both names_.
...
When entering the MLP block, each vector that was originally associated with one of the tokens from the input text is going to go through a short series of operations that'll result in another vector of the same dimension.
...
Before that, let's first finish discussing the rest of this process.
...
When people refer to the **neurons** of a transformer, they're often referring to the values leaving the ReLU. 
...
<Question
  ...
  choice2="Return a simple yes/no answer for the full name Michael Jordan by setting all zero and negative values to zero."
...
And if you have a bias in this case, it's something that's added on regardless of the neuron values. For now, feel free to ignore it, as it's kind of hard to say exactly what it's doing, just like all other parameter-filled objects. We'll label this down projection matrix as $$W_\downarrow$$ and this bias vector as $$B_\downarrow$$.
...
But where it gets interesting is when you relax that constraint a little bit and you tolerate some noise. Say you allow those features to be represented by vectors that aren't exactly perpendicular; they're just nearly perpendicular, maybe between **89** and **91** degrees apart.
...
For visualization purposes, here is a really quick illustration of this using some Python. The program we wrote is going to create a list of **100-dimensional vectors**, each one initialized randomly, and this list is going to contain **10,000 distinct vectors**, so 100 times as many vectors as there are dimensions.

<Figure image="RandomDis.png" width="80%"/>

This plot right here shows the distribution of angles between pairs of these vectors. Because they started at random, those angles could be anything from 0 to 180 degrees, but you'll notice that already, even just for random vectors, there's this heavy bias for the angle to be closer to 90 degrees.
