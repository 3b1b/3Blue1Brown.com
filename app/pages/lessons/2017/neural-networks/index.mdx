...
<Interactive Component={lazy(() => import("./NeuralNetwork.jsx"))} />
...
<Figure image="$lesson/plain-vanilla.png">
  The simple network we're using to identify digits is just a few layers of
...
<Figure image="$lesson/activations.svg">
  Every neuron has an activation between 0.0 and 1.0, sort of analogous to how
...
<Figure image="$lesson/pixel-values.png">
  Each pixel in the original image has a value between 0.0 (black) and 1.0
  (white).
</Figure>
...
<Figure
  image="$lesson/highlight-first-layer.png"
  video="$lesson/pixels-to-neurons.mp4"
  show="video"
>
  The input layer contains 784 neurons, each of which corresponds to a single
...
<Figure image="$lesson/output-layer.png">
  The output layer of our network has 10 neurons. Each neuron corresponds to a
  particular digit that the image could contain.
</Figure>

Take a look at the following image:

<Figure image="$lesson/confused-output-question.png" />
...
<Figure image="$lesson/hidden-layers.png" />
...
<Figure video="$lesson/network-propagation.mp4">
  Watching the activations in each layer propagate through to determine the
...
<Figure image="$lesson/loops-and-lines.png">
...
<Figure image="$lesson/upper-loop-neuron.png" />
...
<Figure image="$lesson/loop-edges.png">
...
<Figure image="$lesson/line-edges.png">
...
<Figure image="$lesson/edge-detection.png">
  Edge detection isn't just for digits! It's a useful step for all kinds of
  image-recognition problems. [Original lion
  image](https://en.wikipedia.org/wiki/File:Lion_waiting_in_Namibia.jpg) by
  Kevin Pluck, licensed under CC BY 2.0
</Figure>
...
<Figure image="$lesson/desired-edge.png">
  We want this one, specific neuron in the second layer to pick up on whether
  the image contains this one, specific edge.
</Figure>
...
<Figure image="$lesson/weights-blue.png" />
...
<Figure image="$lesson/weights-square-blue.png">
  Each weight is associated with one of the 784 input pixels. Arranging the
  weights into this 28x28 grid makes the correlations between the input image
  and the output activation clear.
</Figure>
...
<Figure image="$lesson/weights-attempt-1.png">
  With these weights, the neuron in the second layer will be more activated when
  pixels in this region are more activated.
</Figure>
...
<Figure image="$lesson/weights-attempt-2.png">
  By adding some negative weights above and below, we make sure the neuron is
  most activated when a narrow edge of pixels is turned on, but the surrounding
  pixels are dark.
</Figure>

<Figure image="$lesson/weights-question.png" />
...
<Figure
  image="$lesson/number-line-squish.png"
  video="$lesson/number-line-squish.mp4"
  show="video"
>
  There's no limit to how big or small the weighted sum might be. But our new
  neuron value should be between 0 and 1, so we need to somehow squish the range
  of possible outputs down to size.
</Figure>
...
<Figure image="$lesson/sigmoid.png">
...
<Figure image="$lesson/13002-blue.png">
...
<Figure image="$lesson/annotated-equation.png" />
...
<Figure video="$lesson/matrix-math-videos/activation-vector.mp4" />
...
<Figure video="$lesson/matrix-math-videos/weight-matrix.mp4" />
...
<Figure video="$lesson/matrix-math-videos/weighted-sum.mp4" />
...
<Figure video="$lesson/matrix-math-videos/bias-vector.mp4" />
...
<Figure video="$lesson/matrix-math-videos/sigmoid.mp4" />
...
<Figure video="$lesson/matrix-math-videos/final-equation.mp4">
  This tiny expression represents the computation of all the neurons in the next
  layer based on all the neurons in the previous layer, using the chosen weights
  and biases.
</Figure>
...
<Figure image="$lesson/neural-network-function.png">
  The entire neural network is a function that uses all its weights and biases
  to take in 784 input pixels and spit out 10 output numbers.
</Figure>
...
<Collapsible title="Bonus Note: The Problem With Sigmoids">
...
<Figure image="$lesson/sigmoid-old-school.png" />
