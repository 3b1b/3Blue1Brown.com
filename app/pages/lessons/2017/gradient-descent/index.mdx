...
<Figure image="$lesson/three-3s.jpg">
...
<Figure image="$lesson/pixels-as-input.jpg">
  Each pixel value in the image becomes the activation of a particular neuron in
...
<Figure
  image="$lesson/recap-propagation.jpg"
  video="$lesson/network-propagation.mp4"
>
  The input layer of neurons has activations based on the pixel values of an
  image. The rest of the layers have activations depending on each previous
  layer.
</Figure>
...
<Figure image="$lesson/layer-hypothesis.jpg">
  We hope that the layered structure might allow the problem to be broken into
  smaller steps, from pixels to edges to loops and lines, and finally to digits.
</Figure>
...
<Figure image="$lesson/training-data.jpg">
...
<Figure image="$lesson/training-vs-testing.jpg">
  We use some of the data for training the network and some other data it's
  never seen before for testing it after it's trained.
</Figure>

This process requires a _lot_ of data. Fortunately, the good people behind the [MNIST database](http://yann.lecun.com/exdb/mnist/) have put together a collection of tens of thousands of handwritten digit images, each labeled with the number they are supposed to be, that we can use for free.
...
<Figure image="$lesson/finding-minima.jpg">
  The process of learning is essentially just finding lower and lower points on
  a specific function.
</Figure>
...
<Figure image="$lesson/weights-and-biases.jpg">
...
<Figure image="$lesson/random-trash.jpg">
  Initialized with totally random weights and biases, the network is terrible at
  identifying digits.
</Figure>
...
<Figure image="$lesson/cost-of-difference.jpg">
  We measure how badly the network is doing by comparing its results to the
  correct answer.
</Figure>
...
<Figure image="$lesson/cost-calculation.jpg">
...
<Figure image="$lesson/cost-ranking-question.jpg" />
...
A has a relatively small cost because nine of the ten output values are perfect, and only the output neuron corresponding to the digit 4 is wrong. C has a much higher cost than A because _all_ of its output neurons are wrong, seemingly by quite a lot.

</Question>

### The Cost Over Many Examples

But we aren't just interested in how the network performs on a _single_ image. To really measure its performance, we need to consider the **average** cost over all the tens of thousands of training examples. This average is our measure of how lousy the network is and how bad the computer should feel.
...
<Figure image="$lesson/neural-network-function.jpg">
  The neural network is a function that takes in images and spits out digit
  predictions based on the weights and biases.
</Figure>
...
<Figure image="$lesson/cost-function.jpg">
  The cost function takes in the weights and biases of a network and uses the
  training images to compute a "cost," measuring how bad the network is at
  classifying those training images.
</Figure>
...
<PiCreature emotion="plain">We can do better! Growth mindset!</PiCreature>
...
<Figure image="$lesson/single-input-cost.jpg">
  In this simple example, we'll imagine that the cost function takes just one
  input (although, in reality, ours takes 13,002 inputs). How do we find an
  input that minimizes the cost?
</Figure>
...
<Figure image="$lesson/infeasible-minimum.jpg">
  For a complicated cost function, computing the exact minimum directly isn't
  going to work.
</Figure>
...
<Figure image="$lesson/follow-slope.jpg" video="$lesson/follow-slope.mp4">
  By following the slope (moving in the downhill direction), we approach a local
  minimum.
</Figure>
...
<Figure video="$lesson/balls-rolling-down.mp4">
  Moving the input position according to the slope is a lot like a ball rolling
  down a hill.
</Figure>
...
<Figure video="$lesson/proportional-to-slope.mp4">
  As the slope gets shallower, take smaller steps to avoid overshooting the
  minimum.
</Figure>
...
<Figure image="$lesson/2-input-cost.jpg">
  We can imagine minimizing a function that takes two inputs (which is still not
  13,002 inputs, but it's one step closer). Starting with a random value, look
  for the downhill direction and repeatedly move that way.
</Figure>
...
Those of you familiar with multivariable calculus will know that this vector is called the "gradient", and it tells you which direction you should step to _increase_ the function most quickly.

Naturally enough, taking the negative of that vector gives you the direction to step which _decreases_ the function most quickly. What's more, the length of that gradient vector is an indication for just how steep that steepest slope is.

<Figure image="$lesson/negative-gradient.jpg">
  The gradient, $\nabla C$, gives the uphill direction, so the negative of the
...
<Figure image="$lesson/gradient-descent.jpg">
  Gradient descent just means walking in the downhill direction to minimize the
  cost function.
</Figure>
...
<Figure
  image="$lesson/weights-and-gradient.jpg"
  video="$lesson/gradient-nudge-weights.mp4"
>
  The vector on the left is a list of all the weights and biases for the entire
  network. The vector on the right is the negative gradient, which is just a
  list of all the little nudges you should make to the weights and biases (on
  the left) in order to move downhill.
</Figure>
...
<Figure image="$lesson/gradient-explained.jpg" />
...
<Figure image="$lesson/weight-importance.jpg">
...
<Figure image="$lesson/nudges-on-network.jpg">
  The negative gradient tells us how to change the weights and biases to
  decrease the cost most effectively.
</Figure>
...
[^1]: Remember, this cost function involves an average over _all_ training data, so minimizing it means a better performance on _all_ the training samples.

[^2]:
    One consequence is that the cost function needs to be nice and smooth so that we can efficiently find a local minimum by taking many little steps downhill.
