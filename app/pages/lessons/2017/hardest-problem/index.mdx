...
description: A challenging question of geometry and probability on the Putnam.  The shift in perspective used for the solution here carries with it broader loessons for problem-solving.
date: 2017-12-08
video: OkmNXy7er84
...
<Figure image="$lesson/putnam_overview.png" />
...
<PiCreature emotion="hesitant">It's a hard test</PiCreature>
...
<Figure image="$lesson/2sphere.png" />
...
<Figure image="$lesson/arcs.png" />

It's certainly easier to visualize now, but can you answer the question?

<Question
  question="If $P_1$ and $P_2$ are fixed in place on the circle, with an arc length $\alpha$ between them, and $P_3$ is a point chosen randomly on the circle (with a uniform distribution), what is the probability that the triangle $\Delta P_1 P_2 P_3$ contains the center of the circle?"
  choice1="$\alpha$"
  ...
  answer={2}
>
  As mentioned above, the point $P_3$ needs to land in the arc opposite $P_1$
  and $P_2$. This arc has the same length as the one between $P_1$ and $P_2$,
  which the questions calls $\alpha$, so this probability is the length of that
  arc divided by the full circumference of the circle."
</Question>
...
<Figure video="$lesson/2d_3.mp4" loop />
...
<Figure image="$lesson/green_shell.png" />
...
<Figure image="$lesson/new_objects.png" />
...
<Figure image="$lesson/procedure.png" />
...
<Figure image="$lesson/procedure_2.png" />
...
<Figure image="$lesson/differentiate.png" />
...
<Figure image="$lesson/sphere_procedure.png" />
...
<PiCreature emotion="surprised">Isn't that elegant?</PiCreature>
