...
<Quote name="Anonymous professor">
  Using the chain rule is like peeling an onion: you have to deal with each
  layer at a time, and if it is too big you will start crying.
</Quote>
...
<Figure image="$lesson/figures/chain-rule-dh.svg">
  The fact that $\sin(h)$ is moving left while the $dh$ bump is to the right
  just means that this change $d\left(\sin(h)\right)$ is some negative number.
</Figure>
...
<Figure image="$lesson/figures/chain-rule-example-dh-expanded.svg">
  The derivative of $f(x) = \sin(x^2)$ is reframed by using the transformation
  $h=x^2$.
</Figure>
...
<Collapsible title="Product Rule Explanation">
