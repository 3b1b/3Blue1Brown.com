...
import NeuralNetwork from "../neural-networks/neural-network";
...
<Figure image="$lesson/wrong-answers.png">
  These are some tricky digits! It's not surprising the network struggled here.
</Figure>
...
<Figure image="$lesson/layer-hypothesis.png">
  This was our prediction for the behavior of the layers. Is this what they
  actually do?
</Figure>
...
<Figure image="$lesson/second-layer-weights.png">
  The neurons in the second layer are looking for very loose patterns, but not
  necessarily the little edges we predicted.
</Figure>
...
<Figure image="$lesson/random-input.png">
  The neural network confidently predicts that this random noise is a 5.
</Figure>
...
One fun challenge is to try drawing a 3, and then slowly extending it so it becomes an 8. At some point the network's prediction will change, and as that's happening, watch for which neuron(s) in the 2nd layer are changing, and then to figure out _why_ they change as the 3 becomes an 8.
...
<Figure image="$lesson/confused-network.png">
  The network was only trained on centered, properly-sized images. So when it
  sees anything different, it gets confused.
</Figure>
