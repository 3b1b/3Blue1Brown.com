...
{{< iframe src="//jsfiddle.net/PullJosh/7cgsywf9/embedded/result/dark/?menuColor=000&accentColor=000" >}}

The program above can identify hand-drawn digits 0-9 reasonably accurately. Give a whirl if you haven’t already!

Although it does generally work, it’s not too hard to find situations where it fails. (For example, this poor little network is completely flummoxed by anything which isn’t centered, or isn’t about the right size.)

While more modern neural networks can do a much better job at tasks like this, the network above is simple enough that you can understand exactly what it’s doing and how it was trained with almost no background. It’s also simple enough that you could train it on your own computer, while training more sophisticated networks can require a truly mind-boggling amount of computation.[^1]
...
{{< figure image="plain-vanilla.png" >}}
The simple network we’re using to identify digits is just a few layers of neurons linked together.
{{< /figure >}}

## Neurons

Right now, when I say neuron, all I want you to think is “a thing that holds a number.” Specifically, a number between 0.0 and 1.0. Nural networks are really just a bunch of neurons connected together.
...
{{< figure image="highlight-first-layer.png" video="pixels-to-neurons.mp4" show="video" >}}
The input layer contains 784 neurons, each of which corresponds to a single pixel in the original image.
{{< /figure >}}

When we want to feed the network an image, we’ll set each input neuron’s activation to the brightness of its corresponding pixel.[^3]
...
**Answer:** {{< spoiler >}}It’s having trouble deciding whether the input image is a 4 or a 9.{{< /spoiler >}}
...
{{< figure video="network-propagation.mp4" >}}
...
{{< figure image="loops-and-lines.png" >}}
...
You can imagine how being able to detect edges and patterns would be useful for other image-recognition tasks.

{{< figure image="edge-detection.png" >}}
...
If the neuron in the first layer is on, then a **{{< color color="#4a86e8" >}}positive{{< /color >}}** weight suggests that the neuron in the second layer should also be on, and a **{{< color color="#990000" >}}negative{{< /color >}}** weight suggests that the neuron in the second layer should be off.
...
{{< figure image="weights-square-blue.png" >}}
...
{{< figure image="weights-attempt-1.png" width="500px" >}}
...
{{< figure image="weights-attempt-2.png" width="500px" >}}
...
{{< figure image="number-line-squish.png" video="number-line-squish.mp4" >}}
...
{{< figure image="annotated-equation.png" width="600px" />}}
...
{{< figure video="matrix-math-videos/activation-vector.mp4" width="600px" />}}
...
{{< figure video="matrix-math-videos/weight-matrix.mp4" width="600px" />}}

Then the product $\textcolor{green}{W} a^{(0)}$ is a column vector containing all the weighted sums for the neurons in the next layer.[^4]

{{< figure video="matrix-math-videos/weighted-sum.mp4" width="600px" />}}
...
{{< figure video="matrix-math-videos/bias-vector.mp4" width="600px" />}}
...
{{< figure video="matrix-math-videos/sigmoid.mp4" width="600px" />}}
...
{{< figure video="matrix-math-videos/final-equation.mp4" width="600px" >}}
...
This makes the relevant code much cleaner and much faster, since many libraries optimize the heck out of matrix multiplication.[^5]
...
{{< figure image="neural-network-function.png" >}}
...
[^1]: For example, when OpenAI trained their famous GPT-3 algorithm, it was estimated to require about $12 Million worth of computation.

[^2]: Although neural networks are inspired by the brain, they are by no means identical. It's a lot like the difference between a bird and an airplane. They achieve the same end goal using roughly the same strategies, but with some pretty major differences when it comes to the details.

[^3]: We can't fit all 784 neurons in our diagram, so you're only seeing the first and last neurons, which correspond to the first pixels of the first row and the last pixels of the last row of the image.
...
[^4]: By the way, so much of machine learning comes down to having a good grasp of linear algebra, so if any of you want a nice visual understanding for matrices, and what matrix-vector multiplication means, take a look at {{< lesson-link text="the series I did on linear algebra" link="eola-preview" >}}. Especially {{< lesson-link text="chapter 3" link="linear-transformations" >}}.

[^5]: These days, due to the demand for more machine learning and more powerful networks, there have been huge leaps and bounds in specialized hardware which is aimed at making matrix multiplication much faster, for example, Google's "Tensor processing unit", or TPU.
